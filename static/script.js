...
function focusPlannerInput(event) {
  if (event) event.preventDefault();
  const plannerCard = document.getElementById("planner");
  const userInput = document.getElementById("userInput");

  if (plannerCard) {
    plannerCard.scrollIntoView({ behavior: "smooth" });
  }
  if (userInput) {
    setTimeout(() => {
      userInput.focus();
...
function downloadPDF() {
  ...
  const options = {
    margin: 0.5,
    filename: "Destino-travel-plan.pdf",
    image: {
      type: "jpeg",
      quality: 0.98
    },
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < 135) {
          ...
          ctx.strokeStyle = `rgba(99, 102, 241, ${(1 - dist / 135) * 0.18})`;
          ctx.lineWidth = 0.75;
          ctx.stroke();
        }
      }
