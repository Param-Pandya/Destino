...
try:
    ...
    print(f"PostgreSQL import warning: {exc}", flush=True)
    HAS_POSTGRES = False
...
async def _llm_text_async(system_prompt: str, user_prompt: str) -> str:
    llm = get_llm()
...
5. Estimated airfare range (MUST be formatted in {currency})
6. Peak season pricing warning
7. Booking advice

Return concise travel guidance. All pricing estimates MUST be presented in {currency}.
"""
...
async def flight_agent(state: TravelState):
    ...
    try:
        ...
        prompt = FLIGHT_AGENT_PROMPT.format(
            query=query,
            currency=state.get("preferred_currency", "USD"),
...
"""
        ...
        weather_results = (
            ...
            "Give general seasonal guidance and advise the traveler to verify the forecast."
        )
...
Return:
1. Estimated cost categories (MUST use {currency})
2. Budget risk areas
3. Money-saving suggestions
4. Overall feasibility & total estimated cost in {currency}

Important: All money figures, prices, and cost breakdowns MUST be presented in {currency}.
...
All cost estimates and daily budget allowances MUST be presented in {currency}.
"""
...
async def run_travel_agent_async(user_input: str, thread_id: str | None = None, currency: str = "USD"):
    """Async execution for FastAPI endpoints."""
    if not thread_id:
...
async def resume_travel_agent_async(
    thread_id: str,
    approved: bool,
    feedback: str = "",
):
    """Async thread resumption for FastAPI endpoints."""
    if not thread_id:
...
def run_travel_agent(user_input: str, thread_id: str | None = None, currency: str = "USD"):
    """Sync wrapper for CLI / standalone execution."""
    return asyncio.run(run_travel_agent_async(user_input, thread_id, currency))
